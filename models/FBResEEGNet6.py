...
class FBResEEGNet6(nn.Module):

    def __init__(self, kernels, Samples,  m:int=8, fixed_kernel_size:int=5,  Resblock:int=2, hidden_chans:int=64,
                    in_channels:int=22,  num_classes=9,  fs=250, temporalLayer = 'LogVarLayer', strideFactor= 4,):
        super(FBResEEGNet6, self).__init__()
        ...
        for i, kernel_size in enumerate(list(self.kernels)):
            sep_conv = nn.Conv2d(in_channels=1, out_channels=m, kernel_size=(1,kernel_size),
        ...
        self.sconv = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(in_channels=m, out_channels=hidden_chans, kernel_size=(in_channels,1),
                               stride=(1,1), padding=(0,0), bias=False, groups=m)
        )
        ...
        self.fc = nn.Sequential(nn.Linear(in_features=hidden_chans*strideFactor, out_features=num_classes),
                nn.LogSoftmax(dim=1))
    ...
    def forward(self, x):
        ...
        out = torch.squeeze(self.sconv(out))
        out = self.block(out)  
