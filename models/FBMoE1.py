...
class MoE(nn.Module):
    def __init__(self, nbands, num_experts, m, emb_size, n_features):
        ...
        self.experts = nn.ModuleList([
            nn.Sequential(
                Conv1dWithConstraint(nbands, m*num_experts, groups=nbands, kernel_size=1),
                LinearWithConstraint(n_features, emb_size),
            ) for _ in range(num_experts)
        ])
        
        # 门控网络，决定每个专家的权重
        self.gate = LinearWithConstraint(nbands, m*num_experts)
...
class ConvBlocks(nn.Module):
    def __init__(self, kernels, nbands, m=8,
        ...
        # 空间卷积，用于跨通道特征提取
        self.sconv = Conv2dWithConstraint(in_channels=nbands, out_channels=nbands, kernel_size=(in_channels, 1), 
                               groups=nbands, bias=False)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.bn(self.sconv(x)))  # 空间卷积
