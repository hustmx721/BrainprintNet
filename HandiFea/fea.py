...
def trans_mfccs(wav_data, sample_rate, framesize, mel_band, hop_length):
    ...
    mfccs = mfcc(y=wav_data, sr=sample_rate, n_mels=mel_band, n_fft=framesize, hop_length=hop_length)
    mfccs = mfccs.squeeze()

    return mfccs

# A = pickle.load(open("/data2/tyl/baselines/UserId/dataset/mydata/ori_BCI85.pkl","rb"))

# start_time = time.time()
# # raw = np.load("/data2/tyl/baselines/UserId/dataset/mydata/rest_BCI85.pkl",allow_pickle=True)
# data1, data2 = pickle.load(open(f'/data2/tyl/data/OpenBMI/processed/SSVEP/train.pkl',"rb")), pickle.load(open(f'/data2/tyl/data/OpenBMI/processed/SSVEP/test.pkl',"rb"))

# # tx, ex = raw["ori_train_x"], raw["ori_test_x"]
# # tx, ex = [x.reshape((-1,x.shape[-2],x.shape[-1])) for x in [tx, ex]]
# # ts, es = raw["ori_train_s"], raw["ori_test_s"]
# # ts, es = [s.reshape(-1) for s in [ts, es]]
# tx ,ex = data1["ori_train_x"], data2["ori_test_x"]
# tx, ex = [x.reshape((-1,x.shape[-2],x.shape[-1])) for x in [tx, ex]]
# ts, es = data1["ori_train_s"], data2["ori_test_s"]
# ts, es = [s.reshape(-1) for s in [ts, es]]
# # 乱序
...
# print(f"数据处理完毕,累计用时{time.time()-start_time:.2f}s!")
# print("AR_burg--OpenBMI-SSVEP-数据集")
# [tf, ef] = [AR_burg(x) for x in [tx, ex]]
# [tf, ef] = [fea.reshape((fea.shape[0],-1)) for fea in [tf, ef]]
