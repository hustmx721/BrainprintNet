...
def train(trainloader,valloader,savepath,args):
    ...
    for epoch in tqdm(range(args.epoch),desc="Training:"):
        ...
        if val_acc > best_acc:
            ...
            os.path.join(savepath, f"base_{args.model}_session{args.session_num}_{args.seed}.pth"))
...
def main():
    ...
    sys.stdout = Logger(os.path.join("/mnt/data1/tyl/UserID/logdir", 
                                     f"{cross_task}_" +  f'{args.model}_base.log'))
    ...
    df.to_csv(os.path.join(csv_path, f"base_{args.model}_session{args.session_num}.csv"))
