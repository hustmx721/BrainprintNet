...
from tlutils.network import feat_classifier
from tlutils.loss import CELabelSmooth_raw, Entropy, ReverseLayerF
...
def train(trainloader,valloader, testloader, savepath,args):
    ...
    ad_net = feat_classifier(type="linear", class_num=2, hidden_dim = fea_dim)
    ...
    for epoch in tqdm(range(args.epoch),desc="Training:"):
        ...
        while iter_num < max_iter:
            ...
            reverse_src, reverse_tar = ReverseLayerF.apply(fea_src, alpha), ReverseLayerF.apply(fea_tar, alpha)
            domain_output_s = ad_net(reverse_src)
            domain_output_t = ad_net(reverse_tar)
            domain_label_s = torch.ones(x_src.size()[0]).long().to(device)
            domain_label_t = torch.zeros(x_tar.size()[0]).long().to(device)

            clf_loss = clf_loss_func(logits_src, y_src.long())
            adv_loss = nn.CrossEntropyLoss()(domain_output_s, domain_label_s) + nn.CrossEntropyLoss()(domain_output_t, domain_label_t)
