...
from tlutils.loss import ReverseLayerF, ClassificationMarginDisparityDiscrepancy, MDDClassifier
...
def train(trainloader,valloader, testloader, savepath,args):
    ...
    mdd = ClassificationMarginDisparityDiscrepancy(margin=4.0).to(device)
    mdd.train()

    mdd_classifier = MDDClassifier(backbone_dim=fea_dim, num_classes=args.nclass, bottleneck_dim=50).to(device)
    mdd_classifier.train()

    optimizer_m = torch.optim.Adam(mdd_classifier.parameters(), lr=args.initlr)
    ...
    for epoch in tqdm(range(args.epoch),desc="Training:"):
        ...
        while iter_num < max_iter:
            ...
            out, out_adv = mdd_classifier(fea)
            y_s, y_t = out.chunk(2, dim=0)
            y_s_adv, y_t_adv = out_adv.chunk(2, dim=0)

            transfer_loss = -mdd(y_s, y_s_adv, y_t, y_t_adv)
            clf_loss = clf_loss_func(logits_src, y_src.long())
            total_loss = clf_loss + transfer_loss * args.alignment_weight
