...
from tlutils.loss import JointMultipleKernelMaximumMeanDiscrepancy, GaussianKernel
...
def train(trainloader,valloader, testloader, savepath,args):
    ...
    jmmd_loss_func = JointMultipleKernelMaximumMeanDiscrepancy(
            kernels=(
    ...
    for epoch in tqdm(range(args.epoch),desc="Training:"):
        ...
        while iter_num < max_iter:
            ...
            align_loss = jmmd_loss_func(
                (fea_src, torch.softmax(logits_src, dim=1)), 
                (fea_tar, torch.softmax(logits_tar, dim=1)))
            total_loss = clf_loss + args.alignment_weight * align_loss
