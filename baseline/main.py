...
def train(trainloader,valloader,savepath,args):
    ...
    if args.attack_type is not None:
        if args.attack_type == "AWP":
            attack = AWP(model, optimizer)
        elif args.attack_type == "FGSM":
            attack = FGSM(model, epsilon=args.attack_coef)
    ...
    for epoch in tqdm(range(args.epoch),desc="Training:"):
        ...
        if val_acc > best_acc:
            ...
            os.path.join(savepath, f"base_{args.setsplit}_{args.model}_{args.seed}.pth"))
...
def main():
    ...
    parser.add_argument("--resblock", type=int, default=5)
    ...
    parser.add_argument("--strideFactor", type=int, default=5, help="尺度因子数") # 是否用centerloss训练
    ...
    parser.add_argument("--attack_type", type=str, default=None) # AWP, FGSM
    parser.add_argument("--attack_coef", type=float, default=1e-3) # 攻击系数
    ...
    sys.stdout = Logger(os.path.join("/mnt/data1/tyl/UserID/logdir", 
                                     f"{args.setsplit}_" +  f'base_{args.model}.log'))
    ...
    for idx,seed in enumerate(range(2024, 2029)): 
        ...
        # print(f"strideFactor: {args.strideFactor}")
    ...
    df.to_csv(os.path.join(csv_path, f"base_{args.model}.csv"))
